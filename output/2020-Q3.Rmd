...
prop_of_total <- function (data, var, criteria) {
  stops %>% 
    dplyr::filter(year_quarter == max(year_quarter)) %>% 
    dplyr::count({{var}}) %>% 
...
add_logo <- function (chart, title) {
  ...
  ggpubr::ggarrange(
    ...
    ggpubr::ggarrange(
      ...
      grid::textGrob(
        "More details: bit.ly/stop-search-london",
        x = unit(1, "npc"), 
        hjust = 1,
        gp = grid::gpar(col = "grey33", fontfamily = "Helvetica", fontsize = 8, 
...
save_chart <- function (chart, slug, title, height = 400, width = 600) {
  ggsave(
    filename = str_glue("{report_q}_{slug}.png"), 
...
lsoa <- sf::read_sf("https://opendata.arcgis.com/datasets/42f3aa4ca58742e8a55064a213fb27c9_0.geojson") %>% 
...
london_boroughs <- read_sf("https://opendata.arcgis.com/datasets/3b374840ce1b4160b85b8146b610cd0c_0.geojson") %>%
  janitor::clean_names() %>% 
  filter(str_detect(lad20cd, "^E09")) %>% 
  rename(borough = lad20cd) %>% 
  st_transform(27700)
...
report_period <- str_glue(
  format(as_date(yearmonth(max(stops$date))) - months(2), "%B"), 
  " to ",
  format(max(stops$date), "%B %Y")
)

report_between <- str_replace(report_period, " to ", " and ")
report_span <- str_replace(report_period, " to ", "–")
report_q <- paste(
  year(max(stops$date)),
  case_when(
    month(max(stops$date)) %in% c(1, 2, 3) ~ "q1",
    month(max(stops$date)) %in% c(4, 5, 6) ~ "q2",
    month(max(stops$date)) %in% c(7, 8, 9) ~ "q3",
    month(max(stops$date)) %in% c(10, 11, 12) ~ "q4",
    TRUE ~ NA_character_
  ),
  sep = "_"
)
...
quarterly_counts <- stops %>% 
  count(year_quarter) %>% 
  mutate(
    year_quarter = as_date(year_quarter),
    diff = n - lag(n),
    diff_perc = diff / lag(n)
  )
...
quarterly_counts_feats <- as.list(
  feasts::feat_stl(quarterly_counts$n, .period = 4)
)
...
days_in_quarter <- days_in_month(as_date(max(stops$year_month))) + 
  days_in_month(as_date(max(stops$year_month)) - months(1)) + 
  days_in_month(as_date(max(stops$year_month)) - months(2))
```
...
searches_by_type <- count(stops, year_quarter, object)
```
...
search_result_quarter_counts <- stops %>% 
  mutate(
    positive = !outcome %in% c("no further action", "unknown"),
    year_quarter = as_date(year_quarter)
  ) %>% 
  count(year_quarter, positive) %>% 
...
age_counts <- person_stops %>% 
  filter(year_quarter == max(year_quarter), !is.na(age_range)) %>% 
  tabyl(age_range)

sde_counts <- person_stops %>% 
  filter(year_quarter == max(year_quarter), !is.na(self_defined_ethnicity)) %>% 
  mutate(
    sde_group = case_when(
...
stop_concentration <- lsoa %>% 
  ...
  st_set_geometry(NULL) %>% 
  select(lsoa_code) %>% 
  left_join(count(stops_by_lsoa, lsoa_code, name = "searches")) %>% 
...
low_search_boroughs <- pull(filter(searches_by_borough, searches < searches_by_ward$searches[1]), borough)

places_less_than_top_ward <- case_when(
  length(low_search_boroughs) > 5 ~ str_glue("{number_to_text(length(low_search_boroughs))} entire boroughs"),
  length(low_search_boroughs) > 0 ~ str_glue("the entire boroughs of {vector_to_text(pull(filter(searches_by_borough, searches < searches_by_ward$searches[1]), borough), final_sep = ' or ')}"),
  TRUE ~ str_glue("the {comma(nrow(filter(mutate(arrange(searches_by_ward, searches), csum_searches = cumsum(searches)), csum_searches < searches_by_ward$searches[1])))} wards with the fewest searches combined")
)
```
...
headline1 <- str_glue(
  ...
  comma(pluck(count(filter(stops, year_quarter == max(year_quarter))), "n", 1)),
  " people and vehicles in the three months from {report_period}. The number ",
...
headline2 <- str_replace_all(str_glue(
  percent(pluck(arrange(janitor::adorn_percentages(
    count(filter(stops, year_quarter == max(year_quarter)), object), 
  denominator = "col"), desc(n)), "n", 1)),
  " of searches were for ",
  pluck(arrange(janitor::adorn_percentages(
    count(filter(stops, year_quarter == max(year_quarter)), object), 
  denominator = "col"), desc(n)), "object", 1),
  ", with ",
  percent(1 - tail(search_result_quarter_counts$prop, 1)),
...
{\fontsize{40}{32}\selectfont \mbox{`r report_period`} }
...
chart_trend <- ggplot(monthly_counts_anomaly, aes(year_month, observed)) + 
...
**Between `r report_between`, police officers in London carried out `r comma(pluck(count(filter(stops, year_quarter == max(year_quarter))), "n", 1))` stop-and-searches**, or about `r comma(pluck(count(filter(stops, year_quarter == max(year_quarter))), "n", 1) / 13)` per week. Of those, `r percent(prop_of_total(stops, force, force == "Metropolitan Police Service"))` were conducted by the Metropolitan Police, `r percent(prop_of_total(stops, force, force == "British Transport Police"))` by British Transport Police and `r ifelse(prop_of_total(stops, force, force == "City of London Police") < 0.001, "less than 0.1%", percent(prop_of_total(stops, force, force == "City of London Police")))` by City of London Police. Across the three forces, `r percent(prop_of_total(stops, type, type == "Person search"))` of stops were of pedestrians, `r percent(prop_of_total(stops, type, type == "Person and Vehicle search"))` of people in vehicles and `r percent(prop_of_total(stops, type, type == "Vehicle search"))` of only vehicles.



```{r}
quarterly_change <- str_glue(
  "**", 
  ifelse(nth(quarterly_counts$n, -1) - nth(quarterly_counts$n, -2) > 0, 
         "an increase", "a decrease"),
  " of ",
  percent(abs((nth(quarterly_counts$n, -1) - nth(quarterly_counts$n, -2)) / 
                nth(quarterly_counts$n, -2))),
  " from the previous month**",
  ifelse(
    last(abs(quarterly_counts$diff)) > max(abs(head(tail(quarterly_counts$diff, 6), 5))), 
    # ADAPT THIS TO USE ORDINAL QUARTERS RATHER THAN MONTHS
    str_glue(
      " and the largest quarterly change since ", 
      ifelse(
        nrow(slice(filter(slice(quarterly_counts, 1:n() - 1), abs(diff) > 
                            abs(last(quarterly_counts$diff))), n())) == 0, 
        paste("at least", format(first(quarterly_counts$year_quarter), "%B %Y")),
        format(pluck(slice(
          filter(slice(quarterly_counts, 1:n() - 1), abs(diff) > 
                   abs(last(quarterly_counts$diff))), 
        n()), "year_quarter"), "%B% %Y")
      )
    ), 
    ""
...
change_from_last_quarter <- str_glue(
  ifelse(nth(quarterly_counts$diff, -1) > 0, "an increase", "a decrease"), 
  " of ",
  percent(abs(nth(quarterly_counts$diff, -1) / nth(quarterly_counts$n, -2)))
)

largest_quarterly_change_since <- ifelse(
  last(abs(quarterly_counts$diff)) > max(abs(head(tail(quarterly_counts$diff, 6), 5))), 
  str_glue(
    " and the largest quarterly change since", 
    ifelse(
      nrow(slice(filter(slice(quarterly_counts, 1:n() - 1), abs(diff) > abs(last(quarterly_counts$diff))), n())) == 0, 
      str_glue(" at least the {describe_quarter(first(quarterly_counts$year_quarter))}"),
      str_glue(" the ", describe_quarter(pluck(slice(filter(slice(quarter_counts, 1:n() - 1), abs(diff) > abs(last(quarterly_counts$diff))), n()), "year_month")))
    )
  ), 
  ""
)

change_vs_trend <- ifelse(
  quarterly_counts_feats$trend_strength > 0.5, 
  str_glue(
    " This ", 
    ifelse(nth(quarterly_counts$diff, -1) > 0, "increase", "decrease"), 
    " was ", 
    ifelse(
      (
        quarterly_counts_feats$trend_strength > 0 & nth(quarterly_counts$diff, -1) > 0 | 
        quarterly_counts_feats$trend_strength < 0 & nth(quarterly_counts$diff, -1) < 0
      ), 
      "consistent with", 
      "contrary to"
    ...
    " the long-term ", 
    ifelse(quarterly_counts_feats$linearity > 0, "upward", "downward"), 
    " trend, with stops having ", 
    ifelse(quarterly_counts_feats$linearity > 0, "**increased", "**decreased"), 
    " by ", 
    percent(mean(tail(monthly_counts$diff_perc, 24), na.rm = TRUE)), 
    " per month on average over the past two years.**"
  ), 
  ""
)
```
...
The number of searches carried out in `r report_period` was **`r change_from_last_quarter` from the previous quarter**`r largest_quarterly_change_since` (Figure \@ref(fig:chart-trend-overall)).
`r ifelse(monthly_counts_feats$trend_strength > 0.5, paste(" This", ifelse(nth(quarterly_counts$n, -1) - nth(quarterly_counts$n, -2) > 0, "increase", "decrease"), "was", ifelse((quarterly_counts_feats$trend_strength > 0 & nth(quarterly_counts$n, -1) - nth(quarterly_counts$n, -2) > 0) | (quarterly_counts_feats$trend_strength < 0 & nth(quarterly_counts$n, -1) - nth(quarterly_counts$n, -2) < 0), "consistent with", "contrary to"), "the", ifelse(quarterly_counts_feats$linearity > 0, "upward", "downward"), "trend since 2018, with stops having", ifelse(quarterly_counts_feats$linearity > 0, "**increased", "**decreased"), "by", percent(mean(tail(monthly_counts$diff_perc, 24))), "per month on average over the past two years.**"), "")` Prior to 2018, the number of searches had decreased every year since 2009, dropping by `r percent(abs((last(historical_stops$n) - first(historical_stops$n)) / first(historical_stops$n)))` in nine years.
...
search_type_trends <- stops %>% 
  ...
  filter(
    !object %in% c("other", "unknown"), 
    as_date(year_month) >= as_date(max(stops$year_month)) - years(1)
  ) %>% 
  group_by(object) %>% 
  arrange(year_month) %>% 
  mutate(
...
search_type_changes <- case_when(
  length(search_type_increase) == length(main_types) ~ 
    str_glue("{vector_to_text(search_type_increase)} have all increased"),
  length(search_type_decrease) == length(main_types) ~ 
    str_glue("{vector_to_text(search_type_decrease)} have all decreased"),
  length(search_type_increase) > 0 & length(search_type_decrease) > 0 ~ 
    str_glue("{vector_to_text(search_type_increase)} have increased while searches for {vector_to_text(search_type_decrease)} have decreased{ifelse(length(search_type_increase) + length(search_type_decrease) < length(main_types), paste0(' and ', vector_to_text(setdiff(main_types, search_type_increase))), '')}"), 
  length(search_type_increase) > 0 ~ 
    str_glue("{vector_to_text(search_type_increase)} have increased while searches for {vector_to_text(setdiff(main_types, search_type_increase))} have remained largely static"), 
  length(search_type_decrease) > 0 ~ 
    str_glue("{vector_to_text(search_type_decrease)} have decreased while searches for {vector_to_text(setdiff(main_types, search_type_increase))} have remained largely static"), 
  TRUE ~ "all these items have remained largely static"
)
```



Police officers are empowered to search people for different items -- including drugs, items to use in theft or criminal damage, stolen goods, weapons and even some fireworks -- under different acts of parliament. Although police emphasise that stop and search "[protects Londoners by taking weapons off the streets](https://www.met.police.uk/police-forces/metropolitan-police/areas/about-us/about-the-met/stop-and-search/)", only about one in `r number_to_text(round(1 / pull(filter(janitor::adorn_percentages(filter(searches_by_type, year_quarter == max(year_quarter)), denominator = "col"), object == "weapons"), "n")))` searches between `r report_between` were for weapons -- **`r percent(pluck(arrange(janitor::adorn_percentages(filter(searches_by_type, year_quarter == max(year_quarter)), denominator = "col"), desc(n)), "n", 1))` of searches were for `r pluck(arrange(janitor::adorn_percentages(filter(searches_by_type, year_quarter == max(year_quarter)), denominator = "col"), desc(n)), "object", 1)`** (Figure \@ref(fig:chart-search-types)).
...
chart_types_change <- stops %>% 
  ...
  labs(
    x = NULL, 
    y = "change in searches compared to Jan 2018"
  ) +
  theme_stop_search() +
  theme(
    axis.title.y = element_text(hjust = 0),
...
About `r percent(sum(pull(filter(janitor::adorn_percentages(filter(searches_by_type, year_quarter == max(year_quarter)), denominator = "col"), object %in% main_types), "n")))` of searches are looking for the four main types of contraband: `r vector_to_text(main_types)`. Since 2018, the number of searches for `r search_type_changes` (Figure \@ref(fig:chart-search-types-change)).
...
quarterly_weapon_search_counts <- weapon_search_counts %>% 
  mutate(year_quarter = yearquarter(year_month)) %>% 
  count(legislation, year_quarter, wt = n) %>% 
  mutate(
    year_quarter = as_date(year_quarter),
    diff = n - lag(n),
    diff_perc = diff / lag(n)
...
weapon_trend_s1 <- weapon_search_trends %>% 
  ...
  mutate(text = ifelse(
    ...
    str_glue(
      ...
      percent(abs(mean(pull(filter(weapon_search_counts, legislation == "Police and Criminal Evidence Act 1984 (section 1)", year_month >= max(year_month) - months(24)), diff_perc)))),
...
weapon_previous_anomalies_s1 <- weapon_search_anomolies %>% 
  ...
  mutate(high = remainder > 0) %>% 
  {
    ...
    ifelse(
      anm != "", 
      str_glue(" -- prior to this quarter, PACE section 1 searches had been {anm}"),
      ""
    )
...
weapon_trend_s60 <- weapon_search_trends %>% 
  ...
  mutate(text = ifelse(
    ...
    str_glue(
      ifelse(estimate > 0, "increased", "decreased"), " by about ",
      percent(abs(mean(pull(filter(weapon_search_counts, legislation == "Criminal Justice and Public Order Act 1994 (section 60)", year_month >= max(year_month) - months(24)), diff_perc)))),
      " per month on average"
    ),
    "remained largely static"
  )) %>% pull(text)
...
weapon_previous_anomalies_s60 <- weapon_search_anomolies %>% 
  ...
  mutate(high = remainder > 0) %>% 
  {
    ...
    ifelse(
      anm != "", 
      str_glue("Before that, section 60 searches had been {anm}."),
      ""
    )
...
Searches based on reasonable suspicion the person being searched is carrying a weapon have `r weapon_trend_s1` over the past two years (Figure \@ref(fig:chart-trend-weapons)). In comparison to that trend, the number of these searches in the `r describe_quarter(max(stops$date))` was `r weapon_anomalies_s1` `r weapon_previous_anomalies_s1`.
No-suspicion searches under section 60 have `r weapon_trend_s60` over the past two years, with the number of these searches between `r report_between` `r weapon_anomalies_s60`. `r weapon_previous_anomalies_s60` The number of searches under section 60 is often higher in August because of searches associated with the Notting Hill Carnival.



# Who do police search?

Of the `r comma(pull(count(filter(person_stops, year_quarter == max(year_quarter))), n))` searches of pedestrians and vehicle occupants from `r report_period`, **`r percent(pull(filter(tabyl(filter(person_stops, gender %in% c("female", "male"), year_quarter == max(year_quarter)), gender), gender == "male"), percent))` were searches of men or boys**. Of all people searched, `r percent(sum(pull(filter(age_counts, age_range %in% c("under 10", "10-17")), percent)))` were aged under 18, `r percent(pull(filter(age_counts, age_range == "18-24"), percent))` were between 18 and 24, and `r percent(sum(pull(filter(age_counts, age_range %in% c("25-34", "over 34")), percent)))` were 25 or older. The self-defined ethnicity of the person searched was known for `r percent(1 - pull(filter(sde_counts, is.na(sde_group)), percent))` of searches, of which `r percent(pull(slice(sde_counts, 1), valid_percent))` of people described themselves as `r pull(slice(sde_counts, 1), sde_group)`, `r percent(pull(slice(sde_counts, 2), valid_percent))` as `r pull(slice(sde_counts, 2), sde_group)` and `r percent(pull(slice(sde_counts, 3), valid_percent))` as `r pull(slice(sde_counts, 3), sde_group)`.
...
chart_disparity <- stops_disparity %>% 
  ...
  geom_vline(
    aes(xintercept = x),
    data = tibble(
      x = seq(0, ceiling(max(stops_disparity$search_rate)), by = 5)
    ),
    colour = "white",
    alpha = 0.75
  ) +
  ...
  ungroup() %>% 
  {
    ...
    text <- case_when(
      length(unique(.$group)) == 1 ~ 
        str_glue("the rate of searches was highest for all five of the main ",
                 "types of search for {.$age_label[1]} aged {.$age_range[1]} ",
                 "who identified as {.$ethnic_group[1]}"),
      length(unique(.$sex)) == 1 & length(unique(.$ethnic_group)) == 1 ~ 
        str_glue("the rate of searches was highest for all five of the main ",
                 "types of search for {.$age_label[1]} who identified as ",
                 "{.$ethnic_group[1]}"),
      length(unique(.$sex)) == 1 & length(unique(.$age_range)) == 1 ~ 
        str_glue("the rate of searches was highest for all five of the main ",
                 "types of search for {.$age_label[1]} aged {.$age_range[1]}"),
      pluck(count(., age_range, ethnic_group, sex, sort = TRUE), "n", 1) > 2 ~
        str_glue("the rate of searches was highest for ",
                 "{number_to_text(pluck(arrange(count(., age_range, ethnic_group, sex), desc(n)), 'n', 1))} ",
                 "of the five of the main types of search for ",
    ...
    ifelse(
      str_length(text) > 0,
      str_glue("Of the {number(length(unique(stops_disparity_object$sex)) * length(unique(stops_disparity_object$age_range)) * length(unique(stops_disparity_object$ethnic_group)))} combinations of age, ethnic-group and sex categories present in the data, {text}."),
      ""
    )
...
search_result_trend_text <- case_when(
  nrow(filter(search_result_trends, estimate > 0)) > 0 & nrow(filter(search_result_trends, estimate < 0)) > 0 ~ 
    str_glue("for ", vector_to_text(pull(filter(search_result_trends, estimate > 0), object)), " resulting in a formal outcome increased while the proportion of searches for ", vector_to_text(pull(filter(search_result_trends, estimate < 0), object)), " resulting in a formal outcome decreased"), 
  nrow(filter(search_result_trends, estimate > 0)) > 0 ~ 
    str_glue("for ", vector_to_text(pull(filter(search_result_trends, estimate > 0), object)), " resulting in a formal outcome have", ifelse(nrow(filter(search_result_trends, estimate > 0)) > 2, " all ", " "), "increased"), 
  nrow(filter(search_result_trends, estimate < 0)) > 0 ~ 
    str_glue("for ", vector_to_text(pull(filter(search_result_trends, estimate < 0), object)), " resulting in a formal outcome have", ifelse(nrow(filter(search_result_trends, estimate < 0)) > 2, " all ", " "), "decreased"), 
...
Overall, about `r percent(tail(search_result_quarter_counts$prop, 1))` of searches in the `r describe_quarter(max(stops$date))` resulted in a formal criminal-justice outcome (arrest, charge by post, caution, fixed penalty, community/local resolution or drugs warning), while the remaining **`r percent(1 - tail(search_result_quarter_counts$prop, 1))` of searches resulted in no further action.** Over the past two years, searches for `r head(search_result_order$object, 1)` have been most likely to lead to a formal outcome, while `r percent(1 - tail(search_result_order$prop, 1))` of searches for `r ifelse(tail(search_result_order$object, 1) == "weapons (based on authorisation)", "weapons under a section 60 authorisation", tail(search_result_order$object, 1))` resulted in no further action.

In the past two years, the proportion of searches `r search_result_trend_text` (Figure \@ref(fig:chart-results)). `r ifelse(search_result_month_trends$p.value < 0.05, str_glue("Overall, **the proportion of stops with a formal outcome has ", ifelse(search_result_month_trends$estimate > 0, "risen", "dropped"), " from ", percent(mean(pull(filter(search_result_month_counts, positive, year(year_month) == 2018), prop))), " in 2018 to ", percent(mean(pull(filter(search_result_month_counts, positive, year_month > max(year_month) - months(6)), prop))), " in the past 6 months**."), "")`
...
`r ifelse(sum(outcomes_by_ethnicity_model$sig) / length(outcomes_by_ethnicity_model$sig) > 0.5, str_glue("Just as some ethnic groups are more likely to be stopped than others, the probability of a stop resulting in a formal criminal-justice outcome also varies by ethnicity: over the past six months, the probability of a formal outcome to a search is {vector_to_text(pull(arrange(filter(outcomes_by_ethnicity_model, sig), desc(odds_ratio)), diff_type))}"), paste0("While the rate of searches varies between ethnic groups, the probability of a search resulting in a formal criminal-justice outcome is broadly the same across ethnicities – over the past six months, ", ifelse(sum(outcomes_by_ethnicity_model$sig) > 0, str_glue("the probability of a formal outcome to searches of Black or Asian people is only significantly different from the probabilty of a formal outcome to searches of White people for {vector_to_text(pull(filter(outcomes_by_ethnicity_model, sig), search_type))}"), "the probability of a formal outcome to searches of Black or Asian people was not significantly different from the probabilty of a formal outcome to searches of White people for any of the main search types"), ""))` (Figure \@ref(fig:chart-outcomes-ethnicity)).



(ref:chart-outcomes-ethnicity) Proportion of searches resulting in a formal outcome, `r ifelse(format(max(stops$year_month), '%Y') == format(as_date(max(stops$year_month)) - months(5), '%Y'), format(as_date(max(stops$year_month)) - months(5), '%B'), format(as_date(max(stops$year_month)) - months(5), '%B %Y'))` to `r format(max(stops$year_month), '%B %Y')`
...
Thank you to Ben Bradford, Gavin Hales and Paul Quinton for comments on drafts of this report. Cover photo by [James Padolsey on Unsplash.com](https://unsplash.com/photos/tvPvROBv0F4)



\newgeometry{top = 2cm, inner = 1cm, bottom = 3cm, outer = 1cm}

(ref:chart-map) Location of searches, `r report_period`

```{r chart-map, fig.asp=1, fig.cap="(ref:chart-map)", out.width="19cm"}
chart_map <- grid_stops %>% 
...
tweets[1] <- str_glue(
  "Today @IGCP_UCL releases the first quarterly report on how #stopsearch is ",
  "used by @metpoliceuk @BTPLondon and @CityPolice in London, using #opendata ",
  "from police.uk to bring evidence to the debate on #stopandsearch\n\n",
...
tweets[2] <- str_glue(
  ...
  comma(total_searches / days_in_quarter, accuracy = 10),
  " each day. The number of searches has generally ",
  ifelse(monthly_counts_feats$linearity > 0, "increased", "decreased"),
  " over the past two years.\n\n[{report_q}_01_trend.png]\n\nALT: Bar chart ",
  "showing change in the frequency of stop and search over time"
)
...
tweets[3] <- str_glue(
  ...
  "{pluck(search_count_by_type, 'object', 3)}. Since 2018, searches for ",
  "{search_type_changes}.\n\n[{report_q}_02_types.png and ",
...
weapons_searches_by_power <- weapon_search_counts %>% 
  filter(year_month > max(year_month) - months(3)) %>% 
...
tweets[4] <- str_glue(
  ...
  "{comma(weapons_searches_by_power['(section 1)'])} people they suspected ",
  "of carrying weapons, along with ",
...
tweets[5] <- str_glue(
  "Different groups are searched at different rates – ",
  "{str_remove_all(pluck(arrange(stops_disparity, desc(search_rate)), 'group', 1), '\\\\*\\\\*')} ",
  "were on-average ",
  "{number(pluck(arrange(stops_disparity, desc(search_rate)), 'search_rate', 1) / overall_search_rate)} times ",
  "more likely to be searched than the population at large. The reasons for ",
  "this are likely to be a complex mix of different factors, with no single ",
  "cause.\n\n[{report_q}_05_disparity.png]\n\nALT: Bar chart showing rates at ",
  "which different demographic groups are searched"
)

tweets[6] <- str_glue(
  "{percent(1 - tail(search_result_quarter_counts$prop, 1))} of searches in ",
  "{report_period} resulted in no further action. In the past two years, the ",
...
tweets[7] <- ifelse(
  ...
  str_glue(
    ...
    "is {vector_to_text(pull(arrange(filter(outcomes_by_ethnicity_model, sig), desc(odds_ratio)), diff_type))}",
    "\n\n[{report_q}_07_chart_outcomes_ethnicity.png]"
  ),
  paste0(
    "The likelihood of searches having a formal criminal-justice outcome is ",
    "similar across ethnicities – over the past six months, ",
    ifelse(
      sum(outcomes_by_ethnicity_model$sig) > 0, 
      str_glue(
        "the probability of a formal outcome was similar across ethnic ",
        "groups, except for ",
        "{vector_to_text(pull(filter(outcomes_by_ethnicity_model, sig), search_type))}"
      ),
      "the probability of a formal outcome to searches of Black or Asian people was not significantly different from the probabilty of a formal outcome to searches of White people for any of the main search types"
    ),
    str_glue(
      "\n\n[{report_q}_07_chart_outcomes_ethnicity.png]\n\nALT: Bar charts ", 
...
tweets[8] <- str_glue(
  ...
  "{percent(stop_concentration)} of neighbourhoods – there were more searches ",
  "in {searches_by_ward$ward[1]} ward in {searches_by_ward$borough[1]} than in",
  "{places_less_than_top_ward}\n\n[{report_q}_08_chart_map.png]\n\nALT: Map ",
  "showing where stop and searches are concentrated in London"
)

tweets[9] <- str_glue(
  "The full report is available at https://bit.ly/stop-search-london\n\n",
  "We'll update the report every quarter to provide the latest analysis of",
  "stop and search in London\n\n[{report_q}_00_cover.png]\n\nALT: Cover image ",
  "for stop and search report"
)

tweets %>% 
  paste(collapse = "\n\n-----\n\n") %>% 
  write_file(str_glue("{report_q}_tweets.md"))
```
