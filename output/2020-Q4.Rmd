...
annual_change <- str_glue(
  ifelse(
    nth(rolling_counts$annual, -1) - nth(rolling_counts$annual, -3) > 0,
    "increase",
    "decrease"
  ),
  " of ",
  percent(abs((nth(rolling_counts$annual, -1) - nth(rolling_counts$annual, -13)) / nth(rolling_counts$annual, -13)))
)

change_vs_trend <- ifelse(
  monthly_counts_feats$trend_strength > 0.5, 
  str_glue(
    " This ", 
    ifelse(
      nth(rolling_counts$annual, -1) - nth(rolling_counts$annual, -3) > 0,
    ...
    ifelse(
      (
        monthly_counts_feats$trend_strength > 0 & nth(rolling_counts$annual_diff, -1) > 0 | 
        monthly_counts_feats$trend_strength < 0 & nth(rolling_counts$annual_diff, -1) < 0
      ), 
      "consistent with", 
      "contrary to"
    ...
    " the long-term ", 
    ifelse(monthly_counts_feats$linearity > 0, "upward", "downward"), 
    " trend, with stops having ", 
    ifelse(monthly_counts_feats$linearity > 0, "**increased", "**decreased"), 
    " by ", 
    percent(median(tail(monthly_counts$diff_perc, 24), na.rm = TRUE)), 
    " per month (",
    percent(median(tail(monthly_counts$diff_perc, 24), na.rm = TRUE) * 12),
    " per year) on average over the past two years.**"
  ), 
  ""
)
```
...
The number of searches carried out in `r report_period` was **a year-on-year `r annual_change`** (Figure \@ref(fig:chart-trend-overall)).`r change_vs_trend` Prior to 2018, the number of searches had decreased every year since 2009, dropping by `r percent(abs((last(historical_stops$n) - first(historical_stops$n)) / first(historical_stops$n)))` in nine years.
...
No-suspicion searches under section 60 have `r weapon_trend_s60` over the past 12 months`r weapon_previous_anomalies_s60` The number of searches under section 60 is often higher in August because of searches associated with the Notting Hill Carnival.
...
outcomes_by_ethnicity_model_text <- ifelse(
  ...
  str_glue(
    ...
    "also varies by ethnicity: over the past six months, the probability of a ",
  ...
  str_glue(
    ...
    "the same across ethnicities – over the past six months, ", 
    ifelse(
...
Of the 33 boroughs in London, the most searches in `r report_period` took place in `r stops_by_borough_text`. We can identify search hotspots by dividing London into a grid of equally-sized cells and mapping the density of searches each grid cell (Figure \@ref(fig:chart-map)).
...
s60_by_borough_text <- paste0(
  ...
  case_when(
    ...
    length(setdiff(london_boroughs$borough, s60_by_borough$borough)) > 0 ~ 
      str_glue(
        " Meanwhile, there were no section-60 searches in ",
        to_text(
...
Thank you to Ben Bradford, Gavin Hales and Paul Quinton for comments on drafts of this report. Cover photo by [Collins Lesulie on Unsplash.com](https://unsplash.com/photos/eBJrKE3gng8)
...
tweets[1] <- str_glue(
  "Our updated @IGCP_UCL analysis of #stopandsearch in London, now covering ",
  "the year 2020, shows the number of searches increasing above 300,000 for ",
  "the first year since 2013.\n\n#stopsearch Report: ",
  "http://bit.ly/stop-search-london-2020\n\nThread …\n\n",
...
tweets[2] <- str_glue(
  ...
  ifelse(monthly_counts_feats$linearity > 0, "increased", "decreased"),
  " over the past two years, after substantial declines between 2010 and 2018.\n\n[{report_q}_01_trend.png]\n\nALT: Bar chart ",
...
tweets[7] <- ifelse(
  ...
  paste0(
    ...
    "similar across ethnicities – over the past 12 months, ",
    ifelse(
      sum(outcomes_by_ethnicity_model$sig) > 0, 
      str_glue(
...
tweets %>% 
  paste(collapse = "\n\n-----\n\n") %>% 
  write_file(str_glue("{report_q}/{report_q}_tweets.md"))
```
...
```{r, include=FALSE}
chart_ethnic_disparity <- stops_disparity %>% 
  mutate(
  ...
  labs(
    title = str_glue("<br>**Ethnic disparity in stop and search in London**, {report_period}"),
    subtitle = str_glue(
      "This chart shows the rate of stop and search for people in each ",
      "ethnic group, relative to the rate of stop and search for white ",
      "people <br>of the same age and sex. Values ",
      "<span style='color: #D50032;'>**greater than 1**</span> indicate ",
      "people in that ethnic group are more likely to be searched than white ",
      "<br>people of the same age and sex, values ",
      "<span style='color: #0097A9'>**less than 1**</span> indicate they are ",
