...
stops <- read_rds(here::here("analysis-data/stops.rds"))
```
...
previous_total_searches <- stops %>% 
  filter(date >= date_start - months(12), date <= date_end - months(12)) %>% 
  count() %>% 
  pluck("n", 1)

total_searches_change_num <- (total_searches - previous_total_searches) / previous_total_searches

total_searches_change <- as.character(cut(
  total_searches_change_num,
  breaks = c(-Inf, -0.05, -0.01, 0.01, 0.05, Inf),
  labels = c(
    str_glue("falling by {scales::percent(abs(total_searches_change_num))} to"),
    "falling slightly to",
    "staying roughly constant at",
    "increasing slightly to",
    str_glue("increasing by {scales::percent(abs(total_searches_change_num))} to")
  )
))
...
```{r Twitter cover image, include=FALSE, eval=FALSE}
...
chart_outcomes_ethnicity <- outcomes_by_ethnicity %>% 
  mutate(
    highlight = ethnic_group %in% c("White", "Black", "Asian"),
  ...
  ggplot(aes(
    ...
    y = ethnic_group, 
    alpha = highlight,
    colour = as.logical(align) & highlight, 
    fill = object, 
    label = prop_label, 
    hjust = align
  )) +
  geom_col(alpha = 1, colour = NA, fill = "white") +
  geom_col(colour = NA) +
  geom_text(size = chart_elements$label_text_size, alpha = 1) +
...
grid_kde <- stops_sf %>% 
  hotspot_gistar(cell_size = 750, bandwidth = 750 * 1.5, grid_type = "hex") %>% 
  st_intersection(london_outline) %>% 
  mutate(significant = gistar > 0 & pvalue < 0.05)
...
hotspot_labels <- grid_kde %>% 
  ...
  mutate(
    ...
    id = st_nearest_feature(., london_stations)
  ) %>% 
  left_join(st_drop_geometry(london_stations), by = "id") %>% 
  as_tibble() %>% 
...
Cover photo by [Biel Morro on Unsplash.com](https://unsplash.com/photos/wydEbTWhzTc)
...
tweets[3] <- str_glue(
  ...
  "\n\n[{report_q}_03_types_change.png]\n\nALT: Bar chart showing proportion ",
...
```{r hotspots-by-object, eval=FALSE}
grid_kde_object <- stops_sf %>% 
  filter(object %in% c("weapons", "drugs", "stolen goods", "firearms")) %>% 
  mutate(
    object = case_when(
      object == "weapons" & 
        legislation == "Police and Criminal Evidence Act 1984 (section 1)" ~ 
  ...
  filter(object != "unknown") %>% 
  nest(data = -object) %>% 
  mutate(kde = map(data, hotspot_gistar, cell_size = 750, bandwidth = 750 * 1.5, grid_type = "hex")) %>% 
  select(object, kde) %>% 
  unnest(cols = kde) %>% 
  st_as_sf() %>% 
  st_intersection(london_outline) %>% 
...
grid_kde_object %>% 
  filter(object == "weapons (based on authorisation)") %>% 
  ggplot(aes(colour = kde, fill = kde)) + 
  geom_sf() + 
